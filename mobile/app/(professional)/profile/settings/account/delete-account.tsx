// Ruta: /(professional)/settings/account/delete-account
// Eliminar cuenta — advertencia + confirmación doble (alert) antes de ejecutar.
//
// Flujo:
//   1. Screen muestra lista de lo que se va a perder.
//   2. Tap en "Eliminar mi cuenta" → AppAlert de confirmación (doble confirm).
//   3. Confirma → hook.deleteAccount() corre, store limpia sesión, router
//      redirige al flujo de auth automáticamente por el listener del auth store.
...
export default function DeleteAccountScreen() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      await deleteAccount();
      // No navegamos manualmente: el AuthGate observa el store y redirige a
      // /(auth) cuando la sesión se limpia. Evitamos race conditions con
      // doble navegación.
    } catch (err: unknown) {
